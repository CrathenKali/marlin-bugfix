...
def replace_define(define, value):
    cpp_defines = list(env['CPPDEFINES'])  # Create a copy
    for i, d in enumerate(cpp_defines):
        if isinstance(d, tuple) and d[0] == define:
            cpp_defines[i] = (define, value)
            env['CPPDEFINES'] = cpp_defines
            return
    cpp_defines.append((define, value))
    env['CPPDEFINES'] = cpp_defines
